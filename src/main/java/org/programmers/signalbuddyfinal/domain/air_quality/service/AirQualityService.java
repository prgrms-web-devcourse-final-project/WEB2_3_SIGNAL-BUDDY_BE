...
public class AirQualityService {
  ...
  private void saveToCache(AirQualityResponse airQualityResponse, boolean fresh, String value) {
    String newKey = key + value;
    redisTemplate.opsForValue().set(newKey, new CachedAirQuality(airQualityResponse, fresh), TTL);
  }
  ...
  private ProjCoordinate convertCoordinate(double lng, double lat) {
    return coordinateConverter.convert(lng, lat);
  }

  private ObservatoryResponse getObservatory(double lat, double lng) {
